...
from src.application.runtime.connector_registry import ConnectorRegistry
from src.application.runtime.pipeline_bootstrap import PipelineBootstrapConfig, PipelineDefaults
...
from src.infrastructure.connectors.catalog import CONNECTOR_FACTORIES
...
class UserPipeline:
    def __init__(self, bootstrap: PipelineBootstrapConfig):
        self.bootstrap = bootstrap
        self.profile_loader = ProfileLoader(config_dir=bootstrap.config_dir)
        self.source_profile = bootstrap.source_profile
        self.destination_profile = bootstrap.destination_profile
        self.options_profile = bootstrap.options_profile

        self.source_config = (
            self.profile_loader.load_connector(bootstrap.source_profile) if bootstrap.source_profile else None
        )
        self.destination_profile_config = (
            self.profile_loader.load_connector(bootstrap.destination_profile)
            if bootstrap.destination_profile
            else None
        )
        self.options_config = (
            self.profile_loader.load_options(bootstrap.options_profile) if bootstrap.options_profile else None
        )
        ...
        self.destination = bootstrap.destination or destination_from_profile.get("type")
        if not self.destination:
            raise ValueError("destination or destination_profile is required")

        self.dataset_name = bootstrap.dataset_name or options.get("dataset_name") or options.get("table_name")
        if not self.dataset_name:
            raise ValueError("dataset_name or options.dataset_name/table_name is required")

        self.credentials = bootstrap.credentials or destination_from_profile.get("credentials", {})
        self.destination_config = {
            **destination_from_profile.get("config", {}),
            **dict(bootstrap.destination_config or {}),
        }

        hash_cols_override = bootstrap.hash_columns or []

        self.defaults = PipelineDefaults(
            profile=load_behavior.get("profile", bootstrap.profile),
            primary_key=load_behavior.get("primary_key", bootstrap.primary_key),
            date_column=load_behavior.get("date_column", bootstrap.date_column),
            hash_column=load_behavior.get("hash_column", bootstrap.hash_column),
            hash_columns=list(load_behavior.get("hash_columns", hash_cols_override)),
            extract_chunk_size=runtime.get("extract_chunk_size", bootstrap.extract_chunk_size),
            write_chunk_size=runtime.get("write_chunk_size", bootstrap.write_chunk_size),
            environment=runtime.get("environment", bootstrap.environment),
        )
    ...
    def _build_pipeline_spec(
        ...
        *,
        ...
        return PipelineSpec(
            pipeline_name=self.bootstrap.pipeline_name,
    ...
    def _default_transforms(self, *, source_name: str, hash_columns: list[str]) -> list[TransformSpec]:
        ...
        if hash_columns:
            transforms.append(
                TransformSpec(
                    type="hash_columns",
                    config={"columns": hash_columns, "output_column": self.defaults.hash_column},
                )
            )
        return transforms
    ...
    def _build_runtime_builder(self) -> PipelineRuntimeBuilder:
        ...
        for connector_type, factory in CONNECTOR_FACTORIES.items():
            connector_registry.register(connector_type, factory)
...
def pipeline(*, pipeline_name: str, **kwargs: Any) -> UserPipeline:
    """Build a pipeline from ``pipeline_name`` plus optional fields on :class:`PipelineBootstrapConfig`."""
    return UserPipeline(PipelineBootstrapConfig(pipeline_name=pipeline_name, **kwargs))


__all__ = ["UserPipeline", "pipeline", "PipelineBootstrapConfig"]
