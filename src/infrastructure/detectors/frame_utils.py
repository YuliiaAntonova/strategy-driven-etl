"""Utilities for historized snapshot DataFrames (normalize dates, latest PK row, row prep)."""
...
def prepare_changed_versioned_row(row: pd.Series, existing_created: object) -> pd.Series:
    prepared = row.copy()
    prepared["date_created"] = (
...
def latest_rows_by_primary_key(df: DataFrame, primary_key: str) -> DataFrame:
    ...
    return (
        latest.dropna(subset=[primary_key]).drop_duplicates(subset=[primary_key], keep="last")
    )
